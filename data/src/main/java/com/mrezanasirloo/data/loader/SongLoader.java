...
public class SongLoader {
    ...
    @NonNull
    public static ArrayList<SongDomain> getAllSongs(@NonNull Context context) {
        Cursor cursor = makeSongCursor(context, null, null);
        return getSongs(cursor);
    }

    @NonNull
    public static ArrayList<SongDomain> getSongs(@NonNull final Context context, final String query) {
        Cursor cursor = makeSongCursor(context, AudioColumns.TITLE + " LIKE ?", new String[]{"%" + query + "%"});
        return getSongs(cursor);
    }

    @NonNull
    public static SongDomain getSong(@NonNull final Context context, final int queryId) {
        Cursor cursor = makeSongCursor(context, AudioColumns._ID + "=?", new String[]{String.valueOf(queryId)});
        return getSong(cursor);
    }

    @NonNull
    public static ArrayList<SongDomain> getSongs(@Nullable final Cursor cursor) {
        ArrayList<SongDomain> songDomains = new ArrayList<>();
        if (cursor != null && cursor.moveToFirst()) {
            do {
                songDomains.add(getSongFromCursorImpl(cursor));
    ...
    @NonNull
    public static SongDomain getSong(@Nullable Cursor cursor) {
        SongDomain songDomain;
        if (cursor != null && cursor.moveToFirst()) {
            songDomain = getSongFromCursorImpl(cursor);
        } else {
            songDomain = SongDomain.Companion.getEMPTY_SONG();
        }
        if (cursor != null) {
            cursor.close();
        }
    ...
    @NonNull
    private static SongDomain getSongFromCursorImpl(@NonNull Cursor cursor) {
    ...
    public static Cursor makeSongCursor(@NonNull final Context context, @Nullable final String selection, final String[] selectionValues) {
        return makeSongCursor(context, selection, selectionValues, "" + SortOrder.SongSortOrder.SONG_ALBUM);
    }
